...
export function App() {
  ...
  const viewProjectSessions = (projectId) => {
    ...
    const firstSession = data.runs.find((run) => run.workProjectId === projectId);
  ...
  const displayedDetail = detail?.run?.id === selectedSession && selectedRun
    ? { ...detail, run: { ...detail.run, workProjectId: selectedRun.workProjectId, workProjectName: selectedRun.workProjectName } }
    : detail;
  return <div ref={shellRef} className={`observatory-shell ${activeNav !== 'Sessions' ? 'workspace-shell' : ''}`} style={contextPaneWidth ? { '--context-pane-width': `${contextPaneWidth}px` } : undefined}>
    ...
    {activeNav === 'Sessions' && <>
      <SessionsPane runs={data.runs} workProjects={data.workProjects || []} projectId={sessionProjectId} onProjectChange={setSessionProjectId} sources={data.sessionSources || []} externalSessionCount={data.externalSessionCount || 0} selected={selectedSession} onSelect={setSelectedSession} onManageProjects={() => setActiveNav('Projects')} onAssignWorkProject={async (runId, projectId) => { await api.assignWorkProject(runId, projectId); await reload(); }} onAssignShown={async (runIds, projectId) => { await api.assignWorkProjectRuns(runIds, projectId); await reload(); }} onNewSession={() => setModal('new')} onSync={async () => { await api.syncSessions(); await reload(); }} />
    ...
    {activeNav === 'Projects' && <RepositoriesView repositories={data.repositories} workProjects={data.workProjects || []} hiddenWorkProjectPaths={data.hiddenWorkProjectPaths || []} onViewSessions={viewProjectSessions} onManageSources={() => setActiveNav('Settings')} reload={reload} setError={setError} />}
