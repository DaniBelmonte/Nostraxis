...
export const api = {
  ...
  moveSessions: (runIds, projectId, workItemId = null) => request('/api/organization/assign-runs', { method: 'PUT', body: JSON.stringify({ runIds, projectId, workItemId }) }),
  createWorkItem: (body) => request('/api/work-items', { method: 'POST', body: JSON.stringify(body) }),
  removeWorkItem: (id) => request(`/api/work-items/${encodeURIComponent(id)}`, { method: 'DELETE' }),
  assignWorkItemRuns: (runIds, workItemId) => request('/api/work-items/assign-runs', { method: 'PUT', body: JSON.stringify({ runIds, workItemId }) }),
  selectionAnalysis: (body) => request('/api/selection-analysis', { method: 'POST', body: JSON.stringify(body) }),
